...
def run(weights, model, optimizer, train_cf, clicked_set, user_dict, adj, args, label_map):
    ...
    for epoch in range(args.epochs):
        ...
        for i, batch in enumerate(dataloader):
            ...
            if args.model == "LightGCN" or "CAGCN" in args.model:
                bpr_loss = cal_bpr_loss_light_gcn_reweight(weights[batch['users']], user_embs, pos_item_embs,
                                                           neg_item_embs)
            else:
        ...
        if not epoch % 10:
            ...
            if args.save:
                ...
                print('save success')
    for k in args.topks:
        save_path = os.path.join(os.getcwd(), str(k) + "training_metrics_reweight.png")
...
def obtain_train_weight_power(args, user_id_map):
    ogr_filename = args.path + "/output/" + "activeRatio.txt"

    groups = split_groups_horizontally(ogr_filename, user_id_map, group_num=8)
...
if __name__ == '__main__':
    ...
    best_val = run(w, model, optimizer, train_cf, clicked_set, user_dict, adj, args, label_map)

    # test
    save_model_filename = args.path + \
                          '/trained_model_reweight/' + args.dataset + "/" + args.model + "/" + "_" + str(args.seed) + \
                          "_" + str(args.penalty) + '.pkl'
    ...
    test_results = test(user_embs, user_dict, args, label_map, flag='val')

    save_list = [args.seed, args.penalty]
    for m in ["Recall", "NDCG", "Precision", "Hit_ratio", "F1"]:
        save_list.append(test_results[m][3])
    for m in ["Recommendation_Percentage", "Being_Recommended_Percentage", "System_Score", "Rec_Nums_Inctive"]:
        save_list.append(test_results[m])
